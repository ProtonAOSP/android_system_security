...
public:
    ...
    void update(const sp<IBinder>& token, const KeymasterArguments& params, uint8_t* data,
                size_t dataLength, OperationResult* result) {
        result->resultCode = KM_ERROR_UNIMPLEMENTED;
    }

    void finish(const sp<IBinder>& token, const KeymasterArguments& args, uint8_t* signature,
                size_t signatureLength, OperationResult* result) {
        result->resultCode = KM_ERROR_UNIMPLEMENTED;
    }
